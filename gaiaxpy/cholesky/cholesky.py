...
def get_inverse_square_root_covariance_matrix(input_object: Union[list, Path, str], band: Optional[str] = None):
    ...
    parsed_input_data, extension = InputReader(input_object, get_inverse_square_root_covariance_matrix, None).read()
    if band is None:
...
def get_inverse_covariance_matrix(input_object: Union[list, Path, str], band: Optional[str] = None):
    ...
    parsed_input_data, extension = InputReader(input_object, get_inverse_covariance_matrix, None).read()
    bands_output = []
    if band is None:
        bands_to_process = BANDS
