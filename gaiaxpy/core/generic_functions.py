...
def _validate_pwl_sampling(sampling):
    ...
    if min_value < min_sampling_value or max_value > max_sampling_value:
        raise ValueError(
            f'Wrong value for sampling. Sampling accepts an array of values where the minimum value is {min_sampling_value} and the maximum is {max_sampling_value}.')
...
def _validate_wl_sampling(sampling):
    ...
    if sampling is not None:
        if sampling[0] >= sampling[-1]:
            ...
            raise ValueError(
                f'Wrong value for sampling. Sampling accepts an array of values where the minimum value is {min_value} and the maximum is {max_value}.')
...
def _validate_arguments(default_output_file, given_output_file, save_file):
    ...
    # If the user gave a number different to the default value, but didn't set save_file to True
...
def _get_spectra_type(spectra):
    ...
    """
    spectrum = spectra[0] if isinstance(spectra, Iterable) else spectra
