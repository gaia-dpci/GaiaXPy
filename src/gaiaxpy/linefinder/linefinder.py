...
from gaiaxpy.core.generic_functions import cast_output, get_bands_config, parse_config
...
def _get_configuration(config):
    ...
    """
    transformed_set_dimension = int(config.transformedSetDimension)
    dimension = int(config.dimension)
    if transformed_set_dimension == dimension:
        scale = (config.normalizedRange[1] - config.normalizedRange[0]) / (config.range[1] - config.range[0])
        offset = config.normalizedRange[0] - config.range[0] * scale
        bases_transformation = config.transformationMatrix.reshape(dimension, transformed_set_dimension)
        return bases_transformation, dimension, scale, offset
    else:
        raise Exception('Transformation matrix is not square.')


def _get_config_variables(_config_file):
    bases_config = parse_config(_config_file)
    bands_config = get_bands_config(bases_config)
    bp_tm, bp_n, bp_scale, bp_offset = _get_configuration(bands_config.bpConfig)
    rp_tm, rp_n, rp_scale, rp_offset = _get_configuration(bands_config.rpConfig)
...
def find_lines(input_object: Union[list, Path, str], truncation: bool = False, source_type: str = 'star',
    ...
    bp_tm, bp_n, bp_scale, bp_offset, rp_tm, rp_n, rp_scale, rp_offset = _get_config_variables(hermite_bases_file)
    # Parse input (additional_columns not implemented)
    parsed_input_data, extension = InputReader(input_object, find_lines, truncation, user=username,
...
def find_fast(input_object: Union[list, Path, str], truncation: bool = False, output_path: Union[Path, str] = '.',
    ...
    _check_truncation(truncation)
    bp_tm, bp_n, bp_scale, bp_offset, rp_tm, rp_n, rp_scale, rp_offset = _get_config_variables(hermite_bases_file)
    parsed_input_data, extension = InputReader(input_object, find_lines, truncation,
