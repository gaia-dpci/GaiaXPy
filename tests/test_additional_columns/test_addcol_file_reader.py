...
def test_single_column(setup_data):
    ...
    read_input, _ = InputReader(with_missing_bp_csv_file, generate, False, additional_columns=additional_columns).read()
...
def test_multiple_columns(setup_data):
    ...
    read_input, _ = InputReader(with_missing_bp_fits_file, generate, False,
...
def test_column_already_in_output(setup_data):
    ...
    read_input, _ = InputReader(with_missing_bp_ecsv_file, generate, False,
                                additional_columns=additional_columns).read()
    expected_df, filtered_read_input = parse_dfs_for_test(setup_data['df'], read_input, additional_columns,
                                                          expected_columns)
    pdt.assert_frame_equal(expected_df, filtered_read_input, check_like=True, check_dtype=False)
...
def test_multiple_and_already_in_output(setup_data):
    ...
    read_input, _ = InputReader(with_missing_bp_xml_file, generate, False,
                                additional_columns=additional_columns).read()
    expected_df, filtered_read_input = parse_dfs_for_test(setup_data['df'], read_input, additional_columns,
                                                          expected_columns)
    expected_df = _cast(expected_df)
...
def test_column_renaming_error():
    ...
    with pytest.raises(ValueError):
        read_input, _ = InputReader(with_missing_bp_csv_file, generate, False,
                                    additional_columns=additional_columns).read()
