...
from tests.test_converter.converter_paths import mean_spectrum_csv_file, mean_spectrum_csv_with_missing_int6, \
    ref_sampled_csv, ref_sampled_truncated_csv, missing_band_sampling_solution, optimised_bases_df, \
    converter_csv_solution_0_60_481_df
from tests.utils.utils import get_spectrum_with_source_id_and_xp

# Parsers
parser = InternalContinuousParser()
# Parsed files
parsed_input, _ = parser._parse(mean_spectrum_csv_file)
...
converted_df, positions = convert(mean_spectrum_csv_file, sampling=sampling, save_file=False)
...
class TestGetMethods(unittest.TestCase):
    ...
    def test_get_design_matrices(self):
        ...
        self.assertEqual(list(design_matrices.keys()), keys)
        for key in keys:
            self.assertIsInstance(design_matrices[key], SampledBasisFunctions)
...
class TestTruncation(unittest.TestCase):
    ...
    def test_truncation(self):
        converted_truncated_df, _ = convert(mean_spectrum_csv_file, sampling=sampling, truncation=True, save_file=False)
...
class TestConverterSamplingRange(unittest.TestCase):
    ...
    def test_sampling_equal(self):
        _, _positions = convert(mean_spectrum_csv_file, sampling=sampling, truncation=True, save_file=False)
        npt.assert_array_equal(sampling, _positions)

    def test_sampling_low(self):
        with self.assertRaises(ValueError):
            convert(mean_spectrum_csv_file, sampling=np.linspace(-15, 60, 600), save_file=False)

    def test_sampling_high(self):
        with self.assertRaises(ValueError):
            convert(mean_spectrum_csv_file, sampling=np.linspace(-10, 71, 600), save_file=False)

    def test_sampling_both_wrong(self):
        with self.assertRaises(ValueError):
            convert(mean_spectrum_csv_file, sampling=np.linspace(-11, 71, 600), save_file=False)

    def test_sampling_none(self):
        with self.assertRaises(ValueError):
            convert(mean_spectrum_csv_file, sampling=None, save_file=False)


class TestConverterMissingBand(unittest.TestCase):

    def test_missing_band(self):
        converted_spectra, sampling = convert(mean_spectrum_csv_with_missing_int6, save_file=False)
        npt.assert_array_equal(sampling, np.linspace(0, 60, 600))
        converted_spectra = converted_spectra.iloc[0]
        # Read solution
        solution_values = pd.read_csv(missing_band_sampling_solution, float_precision='high').iloc[0]
