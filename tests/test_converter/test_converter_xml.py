...
from tests.test_converter.converter_paths import optimised_bases_df, converter_csv_solution_0_60_481_df, \
    missing_solution_df
from tests.files.paths import missing_bp_xml_file, mean_spectrum_xml_file, con_ref_sampled_csv_path, \
...
# Parsed files
parsed_input, _ = parser._parse(mean_spectrum_xml_file)
...
converted_df, positions = convert(mean_spectrum_xml_file, sampling=sampling, save_file=False)
...
class TestTruncation(unittest.TestCase):
    ...
    def test_truncation(self):
        converted_truncated_df, _ = convert(mean_spectrum_xml_file, sampling=sampling, truncation=True, save_file=False)
...
class TestConverterSamplingRange(unittest.TestCase):
    ...
    def test_sampling_equal(self):
        _, _positions = convert(mean_spectrum_xml_file, sampling=sampling, truncation=True, save_file=False)
        npt.assert_array_equal(sampling, _positions)

    def test_sampling_low(self):
        with self.assertRaises(ValueError):
            convert(mean_spectrum_xml_file, sampling=np.linspace(-15, 60, 600), save_file=False)

    def test_sampling_high(self):
        with self.assertRaises(ValueError):
            convert(mean_spectrum_xml_file, sampling=np.linspace(-10, 71, 600), save_file=False)

    def test_sampling_both_wrong(self):
        with self.assertRaises(ValueError):
            convert(mean_spectrum_xml_file, sampling=np.linspace(-11, 71, 600), save_file=False)

    def test_sampling_none(self):
        with self.assertRaises(ValueError):
            convert(mean_spectrum_xml_file, sampling=None, save_file=False)
